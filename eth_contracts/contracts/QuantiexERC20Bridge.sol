...
import "./Valset.sol";
import "./BridgeERC20Bank/BridgeERC20Bank.sol";


contract QuantiexERC20Bridge {
    ...
    BridgeERC20Bank public bridgeBank;
    ...
    function setBridgeBank(address payable _bridgeBank) public onlyOperator {
        ...
        hasBridgeBank = true;
        bridgeBank = BridgeERC20Bank(_bridgeBank);
    ...
    function newProphecyClaim(
        ...
        if (_claimType == ClaimType.Burn) {
            require(
                bridgeBank.getLockedFunds(_chainName, _symbol) >= _amount,
            ...
            address bridgeTokenAddress = bridgeBank.getBridgeToken(_chainName, symbol);
            if (bridgeTokenAddress == address(0)) {
                // First lock of this asset, deploy new contract and get new symbol/token address
                tokenAddress = bridgeBank.createNewBridgeToken(_chainName, symbol);
            } else {
     ...
     *       Burn claims unlock tokens stored by BridgeERC20Bank.
     *       Lock claims mint BridgeTokens on BridgeERC20Bank's token whitelist.
     */
     ...
     *       Issues a request for the BridgeERC20Bank to mint new BridgeTokens
     */
    function issueBridgeTokens(uint256 _prophecyID) internal {
        ...
        bridgeBank.mintBridgeTokens(
            prophecyClaim.chainName,
            prophecyClaim.binanceSender,
    ...
    /*
     * @dev: unlockTokens
     *       Issues a request for the BridgeERC20Bank to unlock funds held on contract
     */
